...
create_post() {
    read -p "Enter the blog post URL slug (e.g., 'my-awesome-post'): " slug
    read -p "Enter the blog post title: " title
    
    if [ -z "$slug" ] || [ -z "$title" ]; then
        print_error "Both slug and title are required"
        exit 1
    fi
...
<body>
    ...
    <main>
        <article class="blog-post">
            <header class="post-header">
                <h1>$title</h1>
                <div class="post-meta">
                    <time datetime="$(date +%Y-%m-%d)">Published on $(date +"%B %d, %Y")</time>
                    <span class="reading-time">• 5 min read</span>
                </div>
            </header>

            <div class="post-content">
                <p class="lead">Write your introduction here...</p>
    ...
    <footer>
        <p>&copy; $(date +%Y) TechBlog. All rights reserved.</p>
        <p>Built with ❤️ and served by nginx</p>
    </footer>
</body>
</html>
EOF

    # Copy the post CSS file
    cp "www/linux-a-comprehensive-introduction/post.css" "www/$slug/"
    
    print_success "Blog post template created at www/$slug/index.html"
    print_status "Don't forget to add a link to this post in www/index.html"
}
